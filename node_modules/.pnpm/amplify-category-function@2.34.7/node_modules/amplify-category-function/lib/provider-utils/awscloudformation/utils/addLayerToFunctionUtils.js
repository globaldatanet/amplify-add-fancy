...
const askLayerSelection = async (context, amplifyMeta, runtimeValue, previousSelections = []) => {
    ...
    const layerOptions = lodash_1.default.keys(functionMeta)
        ...
        .filter(key => {
        const runtimes = functionMeta[key].runtimes || layerConfiguration_1.getLayerRuntimes(key);
    ...
    const choices = layerOptions.map(op => ({
        ...
        disabled: layerMigrationUtils_1.getLegacyLayerState(op) !== 0 ? disabledMessage : false,
    }));
    choices.unshift({
        ...
        if (layerVersionChoices.length > 0) {
            ...
            if (previousLayerSelection === undefined || previousLayerSelection.isLatestVersionSelected) {
                ...
                defaultLayerSelection = previouslySelectedLayerVersion
                    ? layerHelpers_1.mapVersionNumberToChoice(previouslySelectedLayerVersion)
                    : defaultLayerVersionPrompt;
            }
            const versionSelection = (await inquirer_1.default.prompt(layerHelpers_1.layerVersionQuestion(layerVersionChoices, versionSelectionPrompt(layerName), defaultLayerSelection))).versionSelection;
