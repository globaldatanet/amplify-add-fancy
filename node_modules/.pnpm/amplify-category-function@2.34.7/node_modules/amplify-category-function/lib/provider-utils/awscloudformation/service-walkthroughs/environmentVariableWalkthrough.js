...
const askEnvironmentVariableQuestions = async (resourceName, environmentVariables = environmentVariablesHelper_1.getStoredEnvironmentVariables(resourceName), skipWalkthrough) => {
    let firstLoop = true;
    for (let operation = skipWalkthrough ? 'abort' : await selectEnvironmentVariableQuestion(lodash_1.default.size(environmentVariables) > 0, firstLoop); operation !== 'abort'; operation = await selectEnvironmentVariableQuestion(lodash_1.default.size(environmentVariables) > 0, firstLoop)) {
        switch (operation) {
            case 'add': {
                const { newEnvironmentVariableKey, newEnvironmentVariableValue } = await addEnvironmentVariableQuestion(environmentVariables, functionSecretsStateManager_1.getLocalFunctionSecretNames(resourceName));
                environmentVariables[newEnvironmentVariableKey] = newEnvironmentVariableValue;
                break;
            }
            case 'update': {
                const { newEnvironmentVariableKey, newEnvironmentVariableValue, targetedKey } = await updateEnvironmentVariableQuestion(environmentVariables, functionSecretsStateManager_1.getLocalFunctionSecretNames(resourceName));
