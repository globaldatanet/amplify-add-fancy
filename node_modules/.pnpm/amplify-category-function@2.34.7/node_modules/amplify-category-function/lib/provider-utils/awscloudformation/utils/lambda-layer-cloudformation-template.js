...
function generateLayerCfnObj(isNewVersion, parameters, versionList = []) {
    const multiEnvLayer = layerHelpers_1.isMultiEnvLayer(parameters.layerName);
    ...
    const layerVersionsToBeRemoved = layerConfiguration_1.getLayerVersionsToBeRemovedByCfn(resourceName, envName);
    ...
    const newLayerVersion = new lambda_1.default.LayerVersion({
        ...
        Content: {
            ...
            S3Key: layerVersion.CreatedDate
                ?
                    `amplify-builds/${packageLayer_1.createLayerZipFilename(resourceName, layerVersion.LogicalName)}`
...
function constructLayerVersionPermissionObjects(layerVersion, layerParameters, shortId, envName) {
    ...
    if (layerVersion.Version) {
        permissions = layerConfiguration_1.getLayerVersionPermissionsToBeUpdatedInCfn(layerParameters.layerName, envName, layerVersion.Version);
    }
