...
function getLayerRuntimes(layerName) {
    try {
        ...
        const legacyState = layerMigrationUtils_1.getLegacyLayerState(layerName);
        if (legacyState !== 0) {
            return layerMigrationUtils_1.readLegacyRuntimes(layerName, legacyState);
        }
        throw e;
    }
}
...
function deleteLayerVersionsToBeRemovedByCfn(layerName, envName) {
    const layerConfig = loadLayerConfigurationFile(layerName);
    amplify_cli_core_1.recursiveOmit(layerConfig, [constants_1.ephemeralField, constants_1.deleteVersionsField, envName]);
...
function saveLayerVersionsToBeRemovedByCfn(layerName, skipVersions, envName) {
    const layerConfig = loadLayerConfigurationFile(layerName);
    lodash_1.default.set(layerConfig, [constants_1.ephemeralField, constants_1.deleteVersionsField, envName], skipVersions);
    writeLayerConfigurationFile(layerName, layerConfig);
}
...
function deleteLayerVersionPermissionsToBeUpdatedInCfn(layerName, envName) {
    const layerConfig = loadLayerConfigurationFile(layerName);
    amplify_cli_core_1.recursiveOmit(layerConfig, [constants_1.ephemeralField, constants_1.updateVersionPermissionsField, envName]);
    writeLayerConfigurationFile(layerName, layerConfig);
}
