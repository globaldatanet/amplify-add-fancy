...
const getFullyQualifiedSecretName = (secretName, functionName, envName) => `${exports.getFunctionSecretPrefix(functionName, envName)}${secretName}`;
exports.getFullyQualifiedSecretName = getFullyQualifiedSecretName;
const getFunctionSecretPrefix = (functionName, envName) => path.posix.join(exports.getEnvSecretPrefix(envName), `AMPLIFY_${functionName}_`);
...
const getEnvSecretPrefix = (envName) => {
    ...
    return path.posix.join('/amplify', exports.getAppId(), envName);
};
exports.getEnvSecretPrefix = getEnvSecretPrefix;
const getFunctionSecretCfnName = (secretName, functionName) => cloudform_types_1.Fn.Join('', [exports.getFunctionSecretCfnPrefix(functionName), secretName]);
