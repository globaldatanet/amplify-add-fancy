...
const packageFunction = async (context, resource) => {
    ...
    if (packageHash) {
        await zipResource_1.zipPackage(packageResult.zipEntries, destination);
    }
    const functionSizeInBytes = await amplify_cli_core_1.getFolderSize(path.join(resourcePath, 'src'));
    let layersSizeInBytes = 0;
    const functionParameters = loadFunctionParameters_1.loadFunctionParameters(resourcePath);
    for (const layer of (functionParameters === null || functionParameters === void 0 ? void 0 : functionParameters.lambdaLayers) || []) {
        if (layer.type === 'ProjectLayer') {
            ...
            layersSizeInBytes += await amplify_cli_core_1.getFolderSize([path.join(layerDirPath, 'lib'), path.join(layerDirPath, 'opt')]);
        }
    }
    if (functionSizeInBytes + layersSizeInBytes > constants_1.lambdaPackageLimitInMB * 1024 ** 2) {
        throw new Error(`Total size of Lambda function ${resource.resourceName} plus it's dependent layers exceeds ${constants_1.lambdaPackageLimitInMB}MB limit. Lambda function is ${amplify_cli_core_1.convertNumBytes(functionSizeInBytes).toMB()}MB. Dependent Lambda layers are ${amplify_cli_core_1.convertNumBytes(layersSizeInBytes).toMB()}MB.`);
    }
