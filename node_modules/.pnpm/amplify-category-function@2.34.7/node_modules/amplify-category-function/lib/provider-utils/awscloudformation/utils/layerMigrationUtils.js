...
exports.removeLayerFromTeamProviderInfo = exports.readLegacyRuntimes = exports.getLegacyLayerState = exports.migrateLegacyLayer = exports.LegacyPermissionEnum = exports.LegacyState = void 0;
...
async function migrateLegacyLayer(context, layerName) {
...
This change requires a migration. Amplify will create a new Lambda layer version even if no layer content changes are made.\n');
    ...
    let runtimeCloudTemplateValues;
    ...
    legacyRuntimeArray.forEach((runtime) => (runtime.cloudTemplateValue = undefined));
    ...
    await Promise.all(layerConfiguration.runtimes.map(async (runtime) => {
        ...
        const runtimePlugin = await functionPluginLoader_1.loadPluginFromFactory(runtime.runtimePluginId, 'functionRuntimeContributorFactory', context);
    ...
    layerConfiguration_1.writeLayerConfigurationFile(layerName, layerConfiguration);
...
function getLegacyLayerState(layerName) {
    const layerDirPath = amplify_cli_core_1.pathManager.getResourceDirectoryPath(undefined, constants_1.categoryName, layerName);
...
function readLegacyRuntimes(layerName, legacyState) {
    const layerDirPath = amplify_cli_core_1.pathManager.getResourceDirectoryPath(undefined, constants_1.categoryName, layerName);
    if (legacyState === 2) {
...
function removeLayerFromTeamProviderInfo(projectRoot, envName, layerName) {
    const nonCfnDataKey = 'nonCFNdata';
    const teamProviderInfo = amplify_cli_core_1.stateManager.getTeamProviderInfo(projectRoot);
    lodash_1.default.unset(teamProviderInfo, [envName, nonCfnDataKey, constants_1.categoryName, layerName]);
    if (lodash_1.default.isEmpty(lodash_1.default.get(teamProviderInfo, [envName, nonCfnDataKey, constants_1.categoryName]))) {
        lodash_1.default.unset(teamProviderInfo, [envName, nonCfnDataKey, constants_1.categoryName]);
        if (lodash_1.default.isEmpty(lodash_1.default.get(teamProviderInfo, [envName, nonCfnDataKey]))) {
            lodash_1.default.unset(teamProviderInfo, [envName, nonCfnDataKey]);
        }
    }
    amplify_cli_core_1.stateManager.setTeamProviderInfo(projectRoot, teamProviderInfo);
}
exports.removeLayerFromTeamProviderInfo = removeLayerFromTeamProviderInfo;
function readLegacyLayerParametersJson(layerDirPath) {
    return amplify_cli_core_1.JSONUtilities.readJson(path.join(layerDirPath, "layer-parameters.json"));
}
function migrateAmplifyProjectFiles(layerName, latestLegacyHash, envName) {
    var _a, _b;
    ...
    lodash_1.default.set(meta, [constants_1.categoryName, layerName, constants_2.versionHash], latestLegacyHash);
