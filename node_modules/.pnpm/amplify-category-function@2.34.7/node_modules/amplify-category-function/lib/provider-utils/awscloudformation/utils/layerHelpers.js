...
function layerNameQuestion(projectName) {
    return {
        ...
        default: `layer${uuid_1.default().split('-')[0]}`,
    };
}
...
function loadStoredLayerParameters(context, layerName) {
    const { permissions, runtimes, description } = layerConfiguration_1.getLayerConfiguration(layerName);
    return {
        layerName,
        runtimes,
        permissions,
...
function isMultiEnvLayer(layerName) {
    ...
    const layerConfiguration = layerConfiguration_1.loadLayerConfigurationFile(layerName, false);
...
async function ensureLayerVersion(context, layerName, previousHash) {
    ...
    await storeResources_1.updateLayerArtifacts(context, layerParameters, { updateLayerParams: false, generateCfnFile: true, updateDescription: false });
    return currentHash;
}
...
const getLayerGlobs = async (resourcePath, resourceName, runtimes, includeResourceFiles) => {
    ...
    for (const runtime of runtimes) {
        ...
        if (runtimeId === 'nodejs') {
            const packageManager = amplify_cli_core_1.getPackageManager(layerCodePath);
            ...
            const contentFilePaths = await globby_1.default([path.join(libPathName, layerExecutablePath, '**', '*')], {
                cwd: resourcePath,
...
const hashLayerVersion = async (layerPath, layerName, includeResourceFiles = false) => {
    const layerConfig = layerConfiguration_1.loadLayerConfigurationFile(layerName, false);
    if (layerConfig) {
        const layerFilePaths = await getLayerGlobs(layerPath, layerName, layerConfig.runtimes, includeResourceFiles);
        const filePaths = await globby_1.default(layerFilePaths, { cwd: layerPath });
        filePaths.sort();
        return filePaths
...
const legacyResourceHashing = async (layerPath) => {
    const files = await globby_1.default(layerResourceGlobs, { cwd: layerPath });
    const hash = files
