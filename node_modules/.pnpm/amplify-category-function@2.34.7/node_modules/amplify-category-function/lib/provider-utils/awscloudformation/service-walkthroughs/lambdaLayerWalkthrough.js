...
async function createLayerWalkthrough(context, parameters = {}) {
    ...
    let { layerName } = await inquirer_1.default.prompt(layerHelpers_1.layerNameQuestion(projectName));
    ...
    for (const permission of layerInputParameters.layerPermissions) {
        switch (permission) {
            case layerParams_1.PermissionEnum.AwsAccounts:
                layerInputParameters.accountIds = await layerHelpers_1.layerAccountAccessPrompt();
                break;
            case layerParams_1.PermissionEnum.AwsOrg:
                layerInputParameters.orgIds = await layerHelpers_1.layerOrgAccessPrompt();
                break;
        }
    }
    parameters.permissions = layerHelpers_1.layerInputParamsToLayerPermissionArray(layerInputParameters);
...
async function updateLayerWalkthrough(context, lambdaToUpdate, parameters) {
    ...
    const hasMigrated = await layerMigrationUtils_1.migrateLegacyLayer(context, parameters.layerName);
    const layerHasDeployed = layerHelpers_1.loadPreviousLayerHash(parameters.layerName) !== undefined;
    let permissionsUpdateConfirmed = false;
    const storedLayerParameters = layerHelpers_1.loadStoredLayerParameters(context, parameters.layerName);
    ...
    if (await context.amplify.confirmPrompt('Do you want to adjust layer version permissions?', true)) {
        ...
        if (layerHasDeployed) {
            ...
            const selectedVersion = (await inquirer_1.default.prompt(layerHelpers_1.layerVersionQuestion(layerVersionChoices, 'Select the layer version to update:'))).versionSelection;
        ...
        const layerInputParameters = await inquirer_1.default.prompt(layerHelpers_1.layerPermissionsQuestion(defaultLayerPermissions));
        for (const permission of layerInputParameters.layerPermissions) {
            switch (permission) {
                case layerParams_1.PermissionEnum.AwsAccounts:
                    layerInputParameters.accountIds = await layerHelpers_1.layerAccountAccessPrompt(defaultAccounts);
                    break;
                case layerParams_1.PermissionEnum.AwsOrg:
                    layerInputParameters.orgIds = await layerHelpers_1.layerOrgAccessPrompt(defaultOrgs);
                    break;
            }
        }
        parameters.permissions = layerHelpers_1.layerInputParamsToLayerPermissionArray(layerInputParameters);
        if (selectedVersionNumber) {
            ...
            layerConfiguration_1.saveLayerVersionPermissionsToBeUpdatedInCfn(parameters.layerName, envName, selectedVersionNumber, parameters.permissions);
        }
    }
...
async function lambdaLayerNewVersionWalkthrough(params, timestampString) {
    const changeLayerPermissions = await inquirer_1.default.prompt(layerHelpers_1.previousPermissionsQuestion());
