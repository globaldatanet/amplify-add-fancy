...
exports.cloneEnvWalkthrough = exports.prePushMissingSecretsWalkthrough = exports.secretValuesWalkthrough = void 0;
...
const secretValuesWalkthrough = async (secretDeltas, envVarNames = [], options = secretValuesWalkthroughDefaultOptions) => {
    ...
    let firstLoop = true;
    for (let operation = await selectOperation(secretDeltaUtilities_1.hasExistingSecrets(secretDeltas), firstLoop); operation !== 'done'; operation = await selectOperation(secretDeltaUtilities_1.hasExistingSecrets(secretDeltas), firstLoop)) {
...
const prePushMissingSecretsWalkthrough = async (functionName, missingSecretNames) => {
    ...
    for (const secretName of missingSecretNames) {
        ...
        secretDeltas[secretName] = amplify_function_plugin_interface_1.setSecret(secretValue);
    }
    return secretDeltas;
};
...
const cloneEnvWalkthrough = async (interactive = true, deltas = {}) => {
    ...
    for (let funcToUpdate = await selectFunctionToUpdate(funcList); !!funcToUpdate; funcToUpdate = await selectFunctionToUpdate(funcList)) {
        await exports.secretValuesWalkthrough(deltas[funcToUpdate], Object.keys(environmentVariablesHelper_1.getStoredEnvironmentVariables(funcToUpdate)), { preConfirmed: true });
    }
    return deltas;
};
...
const updateSecretFlow = async (secretDeltas) => {
    const secretToUpdate = await singleSelectSecret(Object.keys(secretDeltaUtilities_1.getExistingSecrets(secretDeltas)), 'Select the secret to update:');
    const secretValue = await enterSecretValue(secretValueDefaultMessage(secretToUpdate));
    secretDeltas[secretToUpdate] = amplify_function_plugin_interface_1.setSecret(secretValue);
};
const removeSecretFlow = async (secretDeltas) => {
    const secretsToRemove = await multiSelectSecret(Object.keys(secretDeltaUtilities_1.getExistingSecrets(secretDeltas)), 'Select the secrets to delete:');
