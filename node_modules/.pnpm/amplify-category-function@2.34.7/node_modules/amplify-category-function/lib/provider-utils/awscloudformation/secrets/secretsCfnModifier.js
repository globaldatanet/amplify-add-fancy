...
const updateSecretsInCfnTemplate = async (cfnTemplate, secretDeltas, functionName) => {
    ...
    if (!envVarsCfn) {
        lodash_1.default.set(lambdaCfn, ['Properties', 'Environment', 'Variables'], {});
    ...
    Object.entries(secretDeltas).forEach(([secretName, secretDelta]) => {
        switch (secretDelta.operation) {
            ...
            case 'set':
            case 'retain':
                envVarsCfn[secretName] = secretName_1.getFunctionSecretCfnName(secretName, functionName);
                break;
        }
    });
    const hasSecrets = secretDeltaUtilities_1.hasExistingSecrets(secretDeltas);
    if (hasSecrets) {
            ...
            Statement: [
                {
                    ...
                    Resource: cloudform_types_1.Fn.Join('', [
                        ...
                        ':parameter',
                        secretName_1.getFunctionSecretCfnPrefix(functionName),
                        '*',
                    ]),
                },
            ],
