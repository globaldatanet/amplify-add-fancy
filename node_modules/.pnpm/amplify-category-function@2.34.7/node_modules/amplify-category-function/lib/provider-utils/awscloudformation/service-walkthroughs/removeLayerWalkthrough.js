...
async function removeWalkthrough(context, layerName) {
    ...
    const lambdaFunctionsDependentOnLayer = layerHelpers_1.getLambdaFunctionsDependentOnLayerFromMeta(layerName, amplify_cli_core_1.stateManager.getMeta());
    ...
    if (legacyLayerSelectedVersions.length > 0) {
        await deleteLayerVersionsWithSdk(context, layerHelpers_1.getLayerName(context, layerName), legacyLayerSelectedVersions.map(r => r.Version));
    }
    if (layerVersionList.length > totalSelectedVersionsToRemove) {
        if (newLayerSelectedVersions.length > 0) {
            ...
            layerConfiguration_1.saveLayerVersionsToBeRemovedByCfn(layerName, newLayerSelectedVersions.map(r => r.Version), envName);
        }
        const layerParameters = layerHelpers_1.loadStoredLayerParameters(context, layerName);
        storeResources_1.updateLayerArtifacts(context, layerParameters, {
...
async function deleteLayerVersionsWithSdk(context, layerName, versions) {
    ...
    const spinner = ora_1.default('Deleting layer version from the cloud...').start();
    try {
...
function disablePinnedVersions(lambdaFunctionsDependentOnLayer, layerName, layerVersionList) {
    lambdaFunctionsDependentOnLayer.forEach(([lambdaFunctionName]) => {
        const { lambdaLayers: lambdaLayerDependencies } = loadFunctionParameters_1.loadFunctionParameters(amplify_cli_core_1.pathManager.getResourceDirectoryPath(undefined, constants_1.categoryName, lambdaFunctionName));
