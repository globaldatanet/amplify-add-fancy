...
const addLayersToFunctionWalkthrough = async (context, runtime, previousSelections = [], defaultConfirm = false) => {
    ...
    let askArnQuestion;
    ({ lambdaLayers, dependsOn, askArnQuestion } = await addLayerToFunctionUtils_1.askLayerSelection(context, amplify_cli_core_1.stateManager.getMeta(), runtime.value, previousSelections));
    if (askArnQuestion) {
